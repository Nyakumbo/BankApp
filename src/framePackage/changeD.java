...
public class changeD extends javax.swing.JFrame {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (acctype.equalsIgnoreCase("Curent Account")) {
            address = jTextField8.getText();
        }

        try {
            String sql = "Update data set usserName='" + username + "',password='" + pass + "',name='" + name + "',"
                    + "faculty='" + faculty + "',course='" + course + "',university='" + university + "',year='" + year + "',phone='"
                    + phone + "',dateOfBirth='" + date + "',address='" + address + "' where usserName='" + usser + "'";
            ...
            jTextField8.setText("");
            jTextField9.setText("");
            guru.pst = guru.con.prepareStatement(sql);
            guru.pst.executeUpdate();
        } catch (Exception e) {
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        connect guru = new connect();
        guru.connector();
        String user = "", pass = "", accNo = "", name = "", faculty = "", course = "", university = "", year = "", dateOB = "", address = "", phone = "";
        try {
            String sql = "SELECT * FROM data WHERE usserName='" + usser + "'";
            guru.st = guru.con.createStatement();
            guru.rs = guru.st.executeQuery(sql);
            while (guru.rs.next()) {
                user = guru.rs.getString("usserName");
                pass = guru.rs.getString("password");
                acctype = guru.rs.getString("accountType");
                accNo = guru.rs.getString("accountNumber");
                name = guru.rs.getString("name");
                faculty = guru.rs.getString("faculty");
                course = guru.rs.getString("course");
                university = guru.rs.getString("university");
                year = guru.rs.getString("year");
                phone =guru.rs.getString("phone");
                dateOB = guru.rs.getString("dateOfBirth");
                address = guru.rs.getString("address");
        ...
        jComboBox1.setSelectedItem(acctype);
        ...
        jTextField9.setText(dateOB);

        if (acctype.equalsIgnoreCase("Saving Account")) {
            jTextField8.setVisible(false);
            jLabel12.setVisible(false);
        } else if (acctype.equalsIgnoreCase("Curent Account")) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(changeD.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
